...
render answerTruth = Home.headAndBar "Intro - Learn: Lambda Math" $ do
    ...
    h2 "Lambda Calculus Origins" ! A.id "3"
    ...
    h2 "Lambda Calculus Importance" ! A.id "4"
    ...
    h2 "Navigating Lambda Math" ! A.id "5"
    ...
    blockquote $ do
        H.form ! action "/introduction#5" ! method "POST" $ do
            H.label $ do
